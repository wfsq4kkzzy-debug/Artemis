...
# POZNÁMKA: *.db je vyloučeno, ale databáze se zkopíruje ručně později
EXCLUDE_PATTERNS = [
    '__pycache__',
    '*.pyc',
    '*.pyo',
    '*.db',  # Vyloučeno z automatického kopírování, kopíruje se ručně
    '*.sqlite',
    '*.sqlite3',
    '.env',
    '.DS_Store',
...
def copy_version(version: str, description: str = None):
    ...
    # Zkopíruj databázi, pokud existuje (každá verze má svou databázi)
    db_source = base_path / 'library_budget.db'
    if db_source.exists():
        db_dest = version_path / 'library_budget.db'
        try:
            shutil.copy2(db_source, db_dest)
            print(f"   💾 Databáze zkopírována do verze")
        except Exception as e:
            print(f"⚠️  Chyba při kopírování databáze: {e}")
...
# Zkontroluj databázi - každá verze má svou databázi
DB_FILE="library_budget.db"
if [ ! -f "$DB_FILE" ]; then
...
if not exist "library_budget.db" (
    echo 💾 Databáze neexistuje, vytvářím...
    python init_db.py
    echo ✅ Databáze vytvořena
) else (
