...
def detail_rozpoctove_polozky(polozka_id):
    ...
    # Výdaje pro tuto položku - pouze do aktuálního data
    dnes = datetime.utcnow()
    vydaje = Vydaj.query.filter(
        Vydaj.rozpoctova_polozka_id == polozka_id,
        Vydaj.datum <= dnes
    ).order_by(Vydaj.datum.desc()).all()
    
    # Pro zobrazení všech výdajů (i budoucích) pro referenci
    vydaje_vse = Vydaj.query.filter_by(
    ...
    return render_template(
        ...
        polozka=polozka,
        vydaje=vydaje,
        vydaje_vse=vydaje_vse,  # Všechny výdaje pro referenci
