...
class ProjectExecutor:
    ...
    @staticmethod
    @staticmethod
    def add_expense(projekt_id: int, kategorie: str, popis: str, castka: float, **kwargs) -> Dict:
        ...
        try:
            ...
            # Kontrola, že kategorie existuje v rozpočtu
            budget_item = BudgetProjektu.query.filter_by(
                projekt_id=projekt_id,
                kategorie=kategorie
            ).first()
            
            if not budget_item:
                available_categories = db.session.query(BudgetProjektu.kategorie).filter_by(
                    projekt_id=projekt_id
                ).distinct().all()
                categories_list = [c[0] for c in available_categories]
                return {
                    "success": False,
                    "error": f"Kategorie '{kategorie}' není v rozpočtu. Dostupné: {', '.join(categories_list)}",
                }
            ...
            if castka > budget_available:
                return {
                    "success": False,
                    "error": f"Nedostatek rozpočtu! Zbývá jen {budget_available} Kč",
                }
